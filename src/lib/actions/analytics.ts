...
let cachedPredictions: { data: StockPrediction[]; timestamp: number; stateHash: string } | null =
  null;
...
async function fetchAIResponse(prompt: string, maxTokens = 500): Promise<string> {
  ...
  if (zaiApiKey && !zaiApiKey.includes('your-zai')) {
    try {
      ...
      console.error('>>> [Analytics AI] Z.ai Exception:', err);
    }
  }
  ...
  if (groqApiKey && !groqApiKey.includes('your-groq')) {
    try {
      ...
      console.error('>>> [Analytics AI] Groq Exception:', err);
    }
  }
...
export async function getSalesAnalytics(days = 30): Promise<SalesAnalytics | null> {
  ...
  const completed = (orders || []).filter(
    (o) => o.status === 'completed' || o.status === 'pending' || o.status === 'shipped'
  );
  ...
  return {
    ...
    topProducts: Array.from(productMap.entries())
      ...
      .slice(0, 5),
    recentOrders: completed.slice(0, 10).map((o) => ({
      id: o.id,
      product_name: (o.products as any)?.name || 'Unknown',
      quantity: o.quantity,
...
export async function getAIDailyBriefing(
  ...
  if (!process.env.ZAI_API_KEY?.trim() && !process.env.GROQ_API_KEY?.trim())
...
export async function fetchAnomalyInsights(): Promise<string[]> {
  ...
  products.forEach((p: any) => {
    const w1 = orders
      .filter((o: any) => o.product_id === p.id && new Date(o.created_at) >= week1Start)
      .reduce((s: number, o: any) => s + o.quantity, 0);
    const w2 = orders
      .filter(
        (o: any) =>
          o.product_id === p.id &&
          new Date(o.created_at) < week1Start &&
          new Date(o.created_at) >= new Date(Date.now() - 14 * 24 * 60 * 60 * 1000)
      )
      .reduce((s: number, o: any) => s + o.quantity, 0);
    if (w2 > 0) {
  ...
  const anomalyText = anomalies
    .map((a: any) => `${a.name} (${a.change > 0 ? '+' : ''}${a.change}%)`)
    .join(', ');
...
export async function getAIAnalystInsight(
  sales: SalesAnalytics,
  inventory: InventorySummary,
  ...
  if (
    cachedAnalyst &&
    Date.now() - cachedAnalyst.timestamp < 30 * 60 * 1000 &&
    cachedAnalyst.stateHash === stateHash
  )
...
3. IMMEDIATE ACTION PLAN: Give 2 specific, unconventional tactics to execute RIGHT NOW to fix the bottleneck or boost revenue. (e.g., specific dynamic bundling ideas, pricing shifts, or ad focus strategies).`;
  ...
  if (resultText) {
    cachedAnalyst = { text: resultText, timestamp: Date.now(), stateHash };
    return resultText;
  }
...
export async function getShippingRecommendations(destination: string) {
  ...
  const recommendations = rates
    .map((r) => {
      ...
      return {
        partner,
        sellerCost: sellerCost.toFixed(2),
        buyerCharge: buyerCharge.toFixed(2),
        profit: (buyerCharge - sellerCost).toFixed(2),
      };
    })
  ...
  return {
    success: true,
    recommendations,
    aiSummary: aiSummary || `Recommend ${recommendations[0].partner}.`,
  };
}
...
export async function getStockPredictions(): Promise<PredictionResponse> {
  ...
  orders.forEach((o) => {
    salesMap.set(o.product_id, (salesMap.get(o.product_id) || 0) + o.quantity);
  });
  ...
  const itemsToAnalyze = top5Products.map((p) => {
    ...
    return {
      ...
      stock: p.stock,
      dailySalesSeq,
      basicVelocity: totalSold / daysWindow,
    };
  });
  ...
  if (
    cachedPredictions &&
    Date.now() - cachedPredictions.timestamp < 15 * 60 * 1000 &&
    cachedPredictions.stateHash === currentStateHash
  ) {
  ...
  const groqApiKey = process.env.GROQ_API_KEY;
  if (
    (!apiKey || apiKey.includes('your-zai')) &&
    (!groqApiKey || groqApiKey.includes('your-groq'))
  ) {
    return { data: [], lastUpdated: Date.now() };
  }

  // Send raw sequence to AI for curve analysis
  const compressedData = itemsToAnalyze
    .map((i) => `${i.id}|Stock:${i.stock}|Seq:[${i.dailySalesSeq.join(',')}]`)
    .join(';');
...
Output:`;
  ...
  if (apiKey && !apiKey.includes('your-zai')) {
    try {
      ...
      console.error('>>> [Prediction] Z.ai failed');
    }
  }
  ...
  if (!aiText && groqApiKey && !groqApiKey.includes('your-groq')) {
    try {
      ...
      console.error('>>> [Prediction] Groq failed');
    }
  }
  ...
  const result: StockPrediction[] = itemsToAnalyze.map((item) => {
    ...
    const status: 'critical' | 'warning' | 'stable' =
      days <= 3 ? 'critical' : days <= 7 ? 'warning' : 'stable';
...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  let prevMessageTime: number | null = null; // ✅ Track previous message time for sequence latency

  msgList.forEach((m) => {
    const time = new Date(m.created_at).getTime();

    // ✅ Calculate sequence latency (Time between this message and the previous one)
    let sequenceLatency: number | null = null;
    if (prevMessageTime !== null) {
      sequenceLatency = (time - prevMessageTime) / 1000;
    }

    // Calculate customer -> bot reply duration for main KPI average
    let currentReplyTimeSec: number | null = null;
    if (m.sender === 'customer') {
      convLastCustomerTime.set(m.conversation_id, time);
    } else if (m.sender === 'bot' || m.sender === 'owner') {
      const lastTime = convLastCustomerTime.get(m.conversation_id);
      if (lastTime) {
        currentReplyTimeSec = (time - lastTime) / 1000;
        replyDurations.push(currentReplyTimeSec);
        convLastCustomerTime.delete(m.conversation_id);
      }
    }

    // Save to maps with their respective latencies
    latestMessagePerConv.set(m.conversation_id, {
      ...m,
      replyTimeSeconds: currentReplyTimeSec, // Used for Live Feed
      sequenceLatency: sequenceLatency, // Used for Agent Timeline
    });

    prevMessageTime = time; // Update for the next loop iteration
  });

  const volumeMap: Record<
    string,
    { botHandled: number; ownerHandled: number; unanswered: number }
  > = {};
  for (let i = 8; i <= 23; i++)
    volumeMap[i.toString().padStart(2, '0') + ':00'] = {
      botHandled: 0,
      ownerHandled: 0,
      unanswered: 0,
    };
  msgList.forEach((m) => {
    ...
    if (volumeMap[hour]) {
      ...
      else if (m.sender === 'customer' && (m.status === 'unanswered' || !m.status))
        volumeMap[hour].unanswered++;
    }
  });
  const currentHour = new Date().getHours();
  const finalVolumeData = Object.keys(volumeMap)
    .map((time) => {
      const d = volumeMap[time];
      return {
        time,
        botHandled: d.botHandled,
        ownerHandled: d.ownerHandled,
        messages: d.botHandled + d.ownerHandled + d.unanswered,
      };
    })
  ...
  return {
    kpis: {
      totalMessages: msgList.filter((m) => m.sender === 'customer').length,
      botHandled: msgList.filter((m) => m.sender === 'bot').length,
      activeChats: latestMessagePerConv.size,
      ordersToday: (orders?.length || 0) + systemOrders.length,
      unansweredConversations: Array.from(latestMessagePerConv.values()).filter(
        (m) => m.status === 'unanswered' || m.sender === 'customer'
      ).length,
      avgReplyTimeSeconds:
    ...
    liveActivity: Array.from(latestMessagePerConv.values())
      .reverse()
      .slice(0, 5)
      .map((m) => ({
        id: m.id,
        ...
        message: m.text,
        platform: (m as any).platform || ('Instagram' as const),
        status: (m.status as StatusType) || 'unanswered',
        time:
          m.replyTimeSeconds !== null && m.replyTimeSeconds !== undefined
            ? `${m.replyTimeSeconds.toFixed(1)}s`
    ...
    // ✅ UPDATED: Map the real sequence latency for the Agent Timeline
    timeline: msgList
      .filter((m) => m.sender === 'bot' || m.sender === 'system')
      .reverse()
      .slice(0, 5)
      .map((m, idx) => ({
        id: m.id || `tl-${idx}`,
        ...
        type: (m.sender === 'system' ? 'tool' : 'reply') as
          | 'intent'
          | 'tool'
          | 'reply'
          | 'complete',
        latency: m.sequenceLatency ? `${m.sequenceLatency.toFixed(2)}s` : undefined,
      })),
  };
}
...
export interface DashboardData {
  ...
  timeline: {
    ...
    latency: string | undefined; // ✅ FIXED: Added "| undefined" here
  }[];
}

export async function getSidebarStats(): Promise<{ inboxCount: number; inventoryCount: number }> {
  const supabase = await createClient();

  // 1. Get Unanswered Conversations Count
  // We need to look at conversations where the LATEST message is from customer OR status is unanswered
  const { data: messages } = await supabase
    .from('messages')
    .select('conversation_id, sender, status')
    .order('created_at', { ascending: false });

  const latestMessagePerConv = new Map<string, any>();
  (messages || []).forEach((m) => {
    if (!latestMessagePerConv.has(m.conversation_id)) {
      latestMessagePerConv.set(m.conversation_id, m);
    }
  });

  const inboxCount = Array.from(latestMessagePerConv.values()).filter(
    (m) => m.status === 'unanswered' || m.sender === 'customer'
  ).length;

  // 2. Get Low Stock Count
  const { data: products } = await supabase.from('products').select('stock');
  const inventoryCount = (products || []).filter((p) => (p.stock || 0) < 10).length;

  return { inboxCount, inventoryCount };
}
