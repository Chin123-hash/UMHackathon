...
export async function POST(req: Request) {
  try {
    ...
    if (message === "ACTION_CONFIRM_ORDER") {
      ...
      if (stockError) return NextResponse.json({ reply: "Sorry, the item just went out of stock! 🙏" });
    ...
    if (failureCount >= 2) { 
        ...
        await supabase.from("messages").update({ status: 'unanswered' }).eq('conversation_id', conversationId);
        return NextResponse.json({ reply: "Sorry, our AI system is having issues. I've already notified the owner to reply to you manually. 🙏" });
    }
    ...
    const inventoryTable = products?.map(p => 
      `| ID: ${p.id} | NAME: ${p.name} | PRICE: RM${p.price} | STOCK: ${p.stock} units |`
    ).join("\n");
    ...
    if (zaiApiKey && !zaiApiKey.includes("your-zai")) {
      ...
      try {
        ...
        console.error(">>> [DEBUG] Z.ai Exception:", err.name === 'AbortError' ? "Timeout" : err.message);
      }
    }
    ...
    if (!botReply && groqApiKey) {
      ...
      try {
        ...
        console.error(">>> [DEBUG] Groq Exception:", err.name === 'AbortError' ? "Timeout" : err.message);
      }
    }
    ...
    if (botReply) {
      ...
      return NextResponse.json({ reply: "Sorry, the system is a bit busy right now. Please try again? 🙏" });
    }
