...
function AIBriefingPanel({ sales, inventory }: any) {
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try { const res = await getAIDailyBriefing(sales, inventory); setBriefing(res); toast.success("Daily briefing generated!"); } 
    catch (err) { toast.error("Briefing failed"); } finally { setLoading(false); }
  };
  return (
    <div className="bg-white p-5 rounded-card border border-border shadow-card h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2"><FileText size={18} className="text-blue-600" /><h3 className="font-bold">AI Daily Briefing</h3></div>
        <button onClick={handleGenerate} disabled={loading || !sales} className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white text-xs font-semibold rounded-lg hover:bg-blue-700 disabled:opacity-50">
          {loading ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14} />} Generate
        </button>
      </div>
      {loading ? (
        <div className="flex-1 flex items-center justify-center border-2 border-dashed border-gray-200 rounded-lg"><Loader2 className="animate-spin text-blue-400" size={24} /></div>
      ) : briefing ? (
        <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 flex-1 overflow-y-auto"><p className="text-sm text-gray-800 leading-relaxed whitespace-pre-wrap">{briefing}</p></div>
      ) : (
        <div className="flex-1 flex items-center justify-center border-2 border-dashed border-gray-200 rounded-lg"><p className="text-xs text-gray-400">Click generate for a daily business summary.</p></div>
      )}
    </div>
  );
}
...
function AIAnomalyPanel() {
  ...
  const handleDetect = async () => {
    setLoading(true);
    try { const res = await fetchAnomalyInsights(); setInsights(res); toast.success("Anomaly detection complete!"); } 
    catch (err) { toast.error("Detection failed"); } finally { setLoading(false); }
  };
  return (
    <div className="bg-white p-5 rounded-card border border-border shadow-card h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2"><Activity size={18} className="text-orange-600" /><h3 className="font-bold">Anomaly Explainer</h3></div>
        <button onClick={handleDetect} disabled={loading} className="flex items-center gap-1.5 px-3 py-1.5 bg-orange-600 text-white text-xs font-semibold rounded-lg hover:bg-orange-700 disabled:opacity-50">
          {loading ? <Loader2 size={14} className="animate-spin" /> : <AlertTriangle size={14} />} Detect
        </button>
      </div>
      <div className="flex-1 space-y-2 overflow-y-auto">
        {loading ? (
          <div className="h-full flex items-center justify-center border-2 border-dashed border-gray-200 rounded-lg"><Loader2 className="animate-spin text-orange-400" size={24} /></div>
        ) : insights.length === 0 ? (
          <div className="h-full flex items-center justify-center border-2 border-dashed border-gray-200 rounded-lg"><p className="text-xs text-gray-400">Detect sudden spikes/drops in sales.</p></div>
        ) : (
          insights.map((text: string, idx: number) => (
            <div key={idx} className="flex items-start gap-2 p-3 bg-orange-50 border border-orange-100 rounded-lg">
              ...
              <p className="text-xs text-gray-700 leading-relaxed whitespace-pre-wrap">{cleanText(text)}</p>
            </div>
          ))
        )}
      </div>
...
function StockRiskPanel() {
  ...
  const handleScan = async () => {
    ...
    } catch (err) { toast.error("Scan failed"); } finally { setIsLoading(false); }
  };
  ...
  return (
    <div className="bg-white p-5 rounded-card border border-border shadow-card h-full flex flex-col">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 mb-4 text-[10px] text-gray-400 border-t border-dashed border-gray-100 pt-2">
        <span><b className="text-gray-500">Days:</b> Est. until out of stock</span>
        <span><b className="text-gray-500">Pattern:</b> Sales curve shape</span>
        <span><b className="text-gray-500">Conf:</b> Prediction reliability</span>
      </div>
      
      <div className="flex-1 space-y-3 overflow-y-auto pr-1">
        {isLoading ? (
          <div className="h-full flex items-center justify-center"><Loader2 className="animate-spin text-gray-300" size={24} /></div>
        ) : data.length === 0 ? (
          <div className="h-full flex items-center justify-center border-2 border-dashed border-gray-200 rounded-lg px-4 text-center">
            <div>
              ...
              <p className="text-[10px] text-gray-400 mt-1">Click scan to predict stock risks for your top selling items.</p>
            </div>
          </div>
        ) : (
          data.map((p) => (
            <div key={p.id} className={`p-3 rounded-xl border transition-all ${
                p.status === 'critical' ? 'bg-red-50 border-red-200 shadow-sm shadow-red-100' 
                : p.status === 'warning' ? 'bg-orange-50 border-orange-200'
              ...
              <div className="flex items-start justify-between mb-2">
                ...
                <div className={`px-2 py-1 rounded-lg flex flex-col items-center shrink-0 ${
                  p.status === 'critical' ? 'bg-red-100' : p.status === 'warning' ? 'bg-orange-100' : 'bg-gray-100'
                }`}>
                  <span className="text-lg font-black leading-none text-gray-800">{p.daysRemaining || 0}</span>
              ...
              <div className="w-full bg-gray-200/80 h-1.5 rounded-full mb-2">
                <div className={`h-full rounded-full transition-all ${
                    p.status === 'critical' ? 'bg-red-500' : p.status === 'warning' ? 'bg-orange-500' : 'bg-gray-500'
              ...
              <div className="flex items-center justify-between">
                <p className="text-[10px] text-gray-600 italic flex items-center gap-1 flex-wrap">
                  ...
                  <span className="font-semibold not-italic text-indigo-600">{p.aiTrendInsight}</span>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-indigo-100 text-indigo-600">AI</span>
                </div>
              </div>
            </div>
          ))
...
function AIAnalystPanel({ sales, inventory, predictions }: any) {
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try { const res = await getAIAnalystInsight(sales, inventory, predictions || []); setInsight(res); toast.success("AI Analyst report generated!"); } 
    catch (err) { toast.error("AI Insight failed"); } finally { setLoading(false); }
  };
  return (
    <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-card border border-indigo-100 shadow-sm p-6 mb-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={handleGenerate} disabled={loading || !sales} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-semibold rounded-lg hover:bg-indigo-700 active:scale-95 disabled:opacity-50 shadow-md shadow-indigo-200">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-indigo-100 p-5 shadow-inner max-h-[400px] overflow-y-auto whitespace-pre-wrap text-gray-800 leading-relaxed text-sm">{insight}</div>
      ) : (
        <div className="text-center py-8 px-4 border-2 border-dashed border-indigo-200 rounded-xl bg-white/50">
...
export default function SalesAnalyticsPage() {
  ...
  const handleRefresh = () => { mutateSales(); mutateInventory(); toast.info("Refreshing base metrics..."); };
  const lowStockTrend = inventoryData?.lowStockCount ? { value: `${inventoryData.lowStockCount} alerts`, positive: false } : undefined;
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-5 rounded-card border border-border shadow-card h-full">
            ...
            <div className="space-y-4">
              {!salesData ? (
                <div className="h-full flex items-center justify-center py-8"><Loader2 className="animate-spin text-gray-300" size={20} /></div>
        ...
        <div className="bg-white p-5 rounded-card border border-border shadow-card h-full overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
            {!salesData ? (
              <div className="col-span-2 h-32 flex items-center justify-center"><Loader2 className="animate-spin text-gray-300" size={20} /></div>
              ...
              salesData.recentOrders.map((o: any) => (
                <div key={o.id} className="py-2 flex items-center justify-between text-xs border-b border-gray-50 last:border-0">
                  <div><p className="font-medium text-gray-700 truncate max-w-[180px]">{o.product_name}</p><p className="text-[10px] text-gray-400 font-mono uppercase">{o.id.slice(0, 8)}</p></div>
                  <div className="text-right"><p className="font-bold">RM{o.total_price.toFixed(2)}</p><p className="text-[10px] text-gray-400 font-mono">QTY: {o.quantity}</p></div>
                </div>
              ))
            )}
          </div>
